...
eFlow = Tk()
eFlow.title('میدان مغناطیسی داخل حلقه')
...
def calculateB():
    if i.get() == '' or n.get() == '' or r.get() == '':
        ...
        try:
            ...
            nv = int(n.get())
            rv = int(r.get())
            result = mgnt.ringed_mField_intensity(numOfRings = nv, eFlow = iv, radius = rv)
            result *= 10**7
            roundNum = round(result, 3)
            var.set(f"{roundNum} x 10**-7")
            vartxt1.set(txt)
        except:
            messagebox.showerror(title='Operation failed', message='شعاع نمیتواند 0 باشد')
...
Label(eFlow, text=': سیم شعاع', font = FONT).place(x=300, y=202)
...
r = Entry(eFlow, textvariable=p3, width=10)
i.place(x=230, y=113)
n.place(x=230, y=207)
r.place(x=230, y=160)
